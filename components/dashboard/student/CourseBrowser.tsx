...
import { Search, Filter, Star, BookOpen, Clock, Award, Check, ChevronDown } from "lucide-react";

const CATEGORIES = ["All Categories", "Communication", "Leadership", "Problem Solving"];
...
export default function CourseBrowser({ initialCourses }: { initialCourses: Course[] }) {
  ...
  return (
    <>
      <Card className="p-5">
        ...
        {showFilters && (
          <div className="mt-3 flex flex-wrap gap-2 border-t border-grey-20 pt-3">
            {["Beginner to Advanced", "Intermediate", "Under 2 months", "Certificate included"].map((tag) => (
              <span key={tag} className="rounded-full bg-grey-10 px-3 py-1 text-xs font-medium text-grey-70">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div>
        <h2 className="mb-3 mt-6 font-semibold text-primary">
          Courses {filtered.length !== courses.length && <span className="font-normal text-grey-50">({filtered.length} of {courses.length})</span>}
        </h2>

        {filtered.length === 0 && (
          <Card className="p-8 text-center text-sm text-grey-50">No courses match your search.</Card>
        )}
        ...
        <div className="space-y-4">
          {filtered.map((course) => {
            ...
            return (
              <Card key={course.id} className="grid gap-5 p-5 lg:grid-cols-[220px_1fr_280px]">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-1 text-sm">
                    ...
                    <span className="text-grey-50">({(course.reviewCount / 1000).toFixed(1)}k reviews)</span>
                  </div>
                  <p className="mt-2 text-sm leading-relaxed text-grey-70">{course.description}</p>
                  <div className="mt-3 flex flex-wrap gap-3 text-xs text-grey-60">
                    <span className="flex items-center gap-1"><BookOpen className="h-3.5 w-3.5" /> {course.chapterCount} Chapters</span>
                    <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {course.duration}</span>
                    <span className="flex items-center gap-1"><Award className="h-3.5 w-3.5" /> {course.certificateEta}</span>
                  </div>

                  {expanded && (
                    <div className="mt-4 rounded-xl border border-grey-20 p-4">
                      <p className="mb-2 text-sm font-semibold text-primary">Full Curriculum</p>
                ...
                <div className="rounded-xl bg-grey-10 p-4">
                  <p className="mb-2 text-sm font-semibold text-primary">What you&apos;ll learn</p>
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    >
                      {enrollingId === course.id ? "Enrolling…" : course.enrolled ? "Continue" : "Enroll Now"}
                    </Button>
                  </div>
                </div>
              </Card>
