...
export default async function AssessmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const overallPercent = Math.round((assessment.questionsAttempted / assessment.questionsTotal) * 100);

  return (
    <div className="space-y-6">
      <h1 className="font-display text-2xl font-bold text-primary">{assessment.title}</h1>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          <Card className="p-5">
            ...
            <p className="mt-1 text-sm text-grey-60">
              Four short sections covering the soft skills employers screen for most.
            </p>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {assessment.sections.map((s) => (
                <div key={s.label} className="rounded-xl border border-grey-20 p-4">
                  <p className="font-semibold text-primary">{s.label}</p>
                  <p className="mt-1 text-xs text-grey-60">{s.description}</p>
                  <p className="mt-2 text-xs font-medium text-secondary">{s.questionCount} Questions</p>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="space-y-6">
          <Card className="flex flex-col items-center p-5">
            <p className="text-sm font-medium text-grey-60">This week&apos;s progress</p>
            <ProgressRing percent={overallPercent} />
            <p className="mt-1 text-xs text-grey-50">
              {assessment.questionsAttempted} of {assessment.questionsTotal} Questions
            </p>
            <div className="mt-4 grid w-full grid-cols-2 gap-3 text-center text-xs">
              <div className="rounded-lg bg-grey-10 py-2">
                <p className="font-semibold text-primary">{assessment.timeRemainingMinutes} min</p>
                <p className="text-grey-50">Time Remaining</p>
              </div>
              <div className="rounded-lg bg-grey-10 py-2">
                <p className="font-semibold text-primary">{assessment.questionsAttempted}/{assessment.questionsTotal}</p>
          ...
          <Card className="p-5">
            ...
            <div className="space-y-4">
              {assessment.skillCoverage.map((s) => (
                <ProgressBar key={s.label} label={s.label} percent={s.percent} color="bg-success" />
              ))}
            </div>
          </Card>
