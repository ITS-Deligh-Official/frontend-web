...
export default function HomePage() {
  return (
    <main className="bg-[#FBF9F6] text-slate-900">
      {/* Header */}
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-[#FBF9F6]/85 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
            ...
            <a href="#journey" className="transition hover:text-slate-900">How it works</a>
            <a href="#roles" className="transition hover:text-slate-900">Who it&apos;s for</a>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute -bottom-32 -left-20 h-96 w-96 rounded-full bg-amber-100/50 blur-3xl" />
        ...
        <div className="mx-auto grid max-w-7xl gap-16 px-6 pb-20 pt-16 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:pb-28 lg:pt-24">
          <div className="relative z-10">
            ...
            <h1 className="mt-7 text-5xl font-black leading-[1.05] tracking-tight text-slate-900 lg:text-6xl">
            ...
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-slate-600">
              Communication, leadership, teamwork, judgment. ITS Deligh turns the
          ...
          <div id="skills" className="relative z-10 mx-auto w-full max-w-md">
            <div className="rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-100 sm:p-8">
              <div className="mb-2 flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-slate-900">Your Skill Radar</p>
                  <p className="text-xs text-slate-500">Live assessment snapshot</p>
                </div>
              ...
              <div className="relative mx-auto mt-4 aspect-square w-full max-w-[380px]">
                <svg viewBox="0 0 400 400" className="h-full w-full">
                  ...
                  {[
                    ...
                    <circle key={i} cx={x} cy={y} r="4.5" fill="#6D28D9" stroke="white" strokeWidth="1.5" />
                  ))}

                  <circle cx="200" cy="200" r="34" fill="white" stroke="#EDE9FE" strokeWidth="6" />
                  <text x="200" y="196" textAnchor="middle" fontSize="22" fontWeight="800" fill="#4C1D95">87</text>
                  <text x="200" y="212" textAnchor="middle" fontSize="9" fill="#7C3AED" letterSpacing="0.5">SCORE</text>
                </svg>
                ...
                {SKILL_AXES.map((axis) => {
                  ...
                  return (
                    ...
                    >
                      <span className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-100 text-violet-700 ring-4 ring-white">
                        <Icon className="h-4 w-4" />
                      </span>
                      <span className="mt-1 w-20 text-[10px] font-semibold leading-tight text-slate-500">
                        {axis.label}
                      </span>
                    </div>
                  );
      ...
      {/* Roles */}
      <section id="roles" className="border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-6 py-20">
          <div className="max-w-2xl">
            <p className="text-xs font-semibold uppercase tracking-widest text-violet-600">Who it&apos;s for</p>
            <h2 className="mt-3 text-3xl font-bold text-slate-900 lg:text-4xl">
              Built for everyone in the room
            </h2>
            <p className="mt-4 text-slate-600">
              Soft skills aren&apos;t built in isolation. ITS Deligh gives every
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {ROLE_COPY.map(({ role, blurb }) => {
              ...
              return (
                <div
                  key={role.id}
                  className="group rounded-2xl border border-slate-200 p-6 transition hover:-translate-y-1 hover:border-violet-300 hover:shadow-lg"
                >
                  <span className="inline-flex rounded-xl bg-violet-50 p-3 text-violet-700 transition group-hover:bg-violet-600 group-hover:text-white">
                    <Icon className="h-5 w-5" />
                  </span>
                  <h3 className="mt-5 text-lg font-semibold text-slate-900">{role.label}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-slate-500">{blurb}</p>
                </div>
              );
            })}
          </div>
      ...
      <section id="journey" className="mx-auto max-w-7xl px-6 py-20">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-widest text-violet-600">The journey</p>
          <h2 className="mt-3 text-3xl font-bold text-slate-900 lg:text-4xl">
            ...
            return (
              <li key={f.title} className="relative">
                <div className="flex items-center gap-3 md:block">
                  <span className="text-4xl font-black text-violet-100 md:text-5xl">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="inline-flex rounded-xl bg-violet-600 p-2.5 text-white md:mt-3">
                    <Icon className="h-4 w-4" />
                  </span>
                </div>
                ...
                {i < FEATURES.length - 1 && (
                  <div className="mt-6 hidden h-px w-full bg-gradient-to-r from-violet-200 to-transparent md:block" />
                )}
              </li>
            );
          })}
      ...
      <section id="stories" className="border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-6 py-20">
          <p className="text-xs font-semibold uppercase tracking-widest text-violet-600">Real outcomes</p>
          <h2 className="mt-3 text-3xl font-bold text-slate-900 lg:text-4xl">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2">
            {TESTIMONIALS.map((t) => (
              <div key={t.name} className="rounded-3xl bg-[#FBF9F6] p-8 ring-1 ring-slate-100">
      ...
      {/* CTA */}
      <section className="relative overflow-hidden bg-gradient-to-br from-[#14102b] via-[#241650] to-[#3a1c73] py-24">
        <div className="pointer-events-none absolute -left-16 -top-16 h-64 w-64 rounded-full bg-indigo-500/20 blur-3xl" />
        <div className="pointer-events-none absolute -right-10 bottom-0 h-64 w-64 rounded-full bg-fuchsia-500/20 blur-3xl" />
        ...
        <div className="relative z-10 mx-auto max-w-3xl px-6 text-center">
          ...
          <Link href="/signup">
            <Button size="lg" className="mt-10 bg-white px-10 text-violet-700 hover:bg-white/90">
              Get Started Free
      ...
      <footer className="border-t border-slate-200 bg-white py-10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 text-sm text-slate-500 sm:flex-row">
          ...
          <p>&copy; {new Date().getFullYear()} ITS Deligh. All rights reserved.</p>
        </div>
      </footer>
    </main>
  );
