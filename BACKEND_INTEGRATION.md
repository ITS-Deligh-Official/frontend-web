...
`app/api/backend/[...path]/route.ts` is the protected proxy implementation. It only forwards to `BACKEND_API_URL`, sends the access token as `Authorization: Bearer ...`, checks same-origin mutations, limits request bodies to 10 MiB, and retries once after a successful refresh. This is the file to extend if a backend-specific header or a new approved integration is required.
